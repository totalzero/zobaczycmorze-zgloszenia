...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        dane_do_usuniecia = Dane_Dodatkowe.objects.filter(
            zgloszenie__rejs__do__lt=data_graniczna
        ).select_related("zgloszenie", "zgloszenie__rejs")

        liczba = dane_do_usuniecia.count()

        if liczba == 0:
            self.stdout.write(
                self.style.SUCCESS("Brak danych wrazliwych do usuniecia.")
            )
            return

        self.stdout.write(
            f"Znaleziono {liczba} rekordow danych wrazliwych do usuniecia "
            f"(rejsy zakonczone przed {data_graniczna}):"
        )
        ...
        for dane in dane_do_usuniecia:
            ...
            self.stdout.write(
                f"  - {zgloszenie.imie} {zgloszenie.nazwisko} "
                f"(rejs: {rejs.nazwa}, zakonczony: {rejs.do})"
            )

        if dry_run:
            self.stdout.write(
                self.style.WARNING(
                    f"\n[DRY-RUN] Zadne dane nie zostaly usuniete. "
                    f"Uruchom bez --dry-run aby usunac."
                )
            )
            return
