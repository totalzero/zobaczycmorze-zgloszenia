...
def zgloszenie_post_save(sender, instance, created, raw=False, **kwargs):
	# Skip email sending during fixture loading
	if raw:
		return
	if created:
	...
	if old_status is not None and old_status != instance.status:
		link = settings.SITE_URL + reverse("zgloszenie_details", kwargs={"token": instance.token})
		context = {
			"zgl": instance,
...
def wplata_post_save(sender, instance, created, raw=False, **kwargs):
	...
	link = settings.SITE_URL + reverse("zgloszenie_details", kwargs={"token": zgl.token})
	context = {
		"zgl": zgl,
		"wplata": instance,
		"link": link,
...
def ogloszenie_post_save(sender, instance, created, raw=False, **kwargs):
	...
	for z in zgloszenia:
		...
		link = settings.SITE_URL + reverse("zgloszenie_details", kwargs={"token": z.token})
		context = {
			"ogloszenie": instance,
			"zgl": z,
			"rejs": rejs,
